...
SELECT A_TABLE.APNT_NO, B_TABLE.PT_NAME, A_TABLE.PT_NO,A_TABLE.MCDP_CD, A_TABLE.DR_NAME, A_TABLE.APNT_YMD 
FROM (SELECT A.APNT_NO, A.PT_NO, A.MCDP_CD,A.MDDR_ID,A.APNT_YMD,B.DR_NAME FROM (SELECT APNT_NO, PT_NO, MCDP_CD,MDDR_ID,APNT_YMD FROM APPOINTMENT WHERE (DATE_FORMAT(APNT_YMD,'%Y-%m-%d')='2022-04-13') AND (APNT_CNCL_YN='N') AND (MCDP_CD='CS')) AS A INNER JOIN (SELECT DR_NAME, DR_ID FROM DOCTOR) AS B ON A.MDDR_ID=B.DR_ID) AS A_TABLE 
INNER JOIN (SELECT PT_NO, PT_NAME FROM PATIENT) AS B_TABLE 
...
CASE 
WHEN B.DURATION_TYPE IS NULL THEN A.DAILY_FEE*B.DATE_DIFF
WHEN B.DURATION_TYPE IS NOT NULL THEN ROUND((A.DAILY_FEE*B.DATE_DIFF)*(100-C.DISCOUNT_RATE)*0.01)
END AS FEE
FROM (SELECT CAR_ID, CAR_TYPE, DAILY_FEE FROM CAR_RENTAL_COMPANY_CAR WHERE CAR_TYPE='트럭') AS A 
LEFT JOIN (SELECT HISTORY_ID,CAR_ID,
            CASE
            WHEN (DATEDIFF(END_DATE,START_DATE)+1<7) THEN NULL
            WHEN (DATEDIFF(END_DATE,START_DATE)+1>=7) AND (DATEDIFF(END_DATE,START_DATE)+1<29) THEN '7일 이상'
            WHEN (DATEDIFF(END_DATE,START_DATE)+1>=30) AND (DATEDIFF(END_DATE,START_DATE)+1<90) THEN '30일 이상'
            WHEN DATEDIFF(END_DATE,START_DATE)+1>=90 THEN '90일 이상'
            END AS DURATION_TYPE,
            DATEDIFF(END_DATE,START_DATE)+1 AS DATE_DIFF FROM CAR_RENTAL_COMPANY_RENTAL_HISTORY) AS B ON A.CAR_ID=B.CAR_ID LEFT JOIN (SELECT * FROM CAR_RENTAL_COMPANY_DISCOUNT_PLAN WHERE CAR_TYPE='트럭') AS C ON B.DURATION_TYPE=C.DURATION_TYPE
ORDER BY 2 DESC, 1 DESC
...
SELECT COUNT(*) AS FISH_COUNT FROM FISH_INFO WHERE LEFT(TIME,4)='2021'
